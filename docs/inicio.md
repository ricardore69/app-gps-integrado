# Documentación técnica – inicio.js

## 📌 Propósito del módulo
El archivo `inicio.js` controla la lógica de la pantalla inicial de GeoZapp.  
Desde aquí se maneja:

- Autenticación del usuario
- Carga del perfil desde Firestore
- Gestión de levantamientos existentes
- Creación de nuevos levantamientos
- Navegación hacia levantamiento.html
- Cierre de sesión

Es el punto de entrada funcional después del login.
...
- Firebase Firestore (query, where, getDocs, doc, collection, addDoc, getDoc, setDoc)
...
const nuevoLevantamientoForm = document.getElementById("nuevoLevantamientoForm");

// Variables de estado
let usuarioActual = null;
let modoSeleccionado = "levantamiento";
...
onAuthStateChanged(auth, async (user) => {
  ...
  localStorage.removeItem("levantamientoActivo");
  ...
  const querySnapshot = await getDocs(levantamientosRef);
...
selectLevantamientos.addEventListener("change", (e) => {
  ...
  if (selected === "nuevo") {
    ...
    nuevoLevantamientoForm.style.display = "none";
  }
});
